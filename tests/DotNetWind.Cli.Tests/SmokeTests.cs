...
{
    [Theory]
    [InlineData("blazor", "BlazorApp")]
    [InlineData("blazorwasm", "WasmApp")]
    [InlineData("mvc", "MvcApp")]
    [InlineData("webapp", "RazorPagesApp")]
    [InlineData("razorclasslib", "RclApp")]
    public async Task Init_WithGeneratedProject_ConfiguresTailwindFiles(string template, string projectName)
    {
        ...
        await RunAsync(
            "dotnet",
            $"run --no-build --configuration {GetConfiguration()} --project {Quote(GetCliProjectPath())} -- init --project {Quote(projectPath)} --skip-npm-install --skip-build",
            GetRepositoryRoot());
    ...
    private static string GetCliProjectPath() =>
        Path.Combine(GetRepositoryRoot(), "src", "DotNetWind.Cli", "DotNetWind.Cli.csproj");
