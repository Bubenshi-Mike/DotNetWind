...
public sealed class TailwindInitializerTests
{
    ...
    [Fact]
    public async Task InitializeAsync_WhenRefreshingPackageEntries_UsesRefreshPath()
    {
        var fileSystem = new FakeFileSystem();
        var packageJsonManager = Substitute.For<IPackageJsonManager>();
        var projectFileUpdater = Substitute.For<IProjectFileUpdater>();
        ...
        packageJsonManager.RefreshManagedEntriesAsync(
                Arg.Any<string>(),
                Arg.Any<string>(),
                Arg.Any<string>(),
                Arg.Any<CancellationToken>())
        ...
        var options = new SetupOptions(
            SkipNpmInstall: true,
            SkipBuild: true,
            RefreshPackageJsonEntries: true);

        var result = await initializer.InitializeAsync(project, paths, options);

        result.IsSuccess.ShouldBeTrue();
        await packageJsonManager.Received(1).RefreshManagedEntriesAsync(
            paths.PackageJsonPath,
            "Styles/tailwind.css",
            "wwwroot/css/style.css",
            Arg.Any<CancellationToken>());
        await packageJsonManager.DidNotReceiveWithAnyArgs().CreateOrMergeAsync(
            default!,
            default!,
            default!,
            default);
